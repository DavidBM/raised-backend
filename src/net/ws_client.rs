...
impl WsClient {
    ...
    fn extract_data_special_cases(&self, message: packets::PacketType, _packet: &str) {
        match message.t.as_ref() {
            "stay" => self
                .sender
                .send(ClientPacket::Stay)
                .expect("Cannot send to game client decoded message: stay"),
...
impl ws::Handler for WsClient {
    ...
    fn on_close(&mut self, code: ws::CloseCode, reason: &str) {
        ...
        match code {
            ...
            ws::CloseCode::Abnormal => trace!(
                "Closing ws handshake failed! Unable to obtain closing status from client: {:?} {:?} {:?}",
                &self,
                code,
                reason
            ),
